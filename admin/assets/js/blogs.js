...
var blog_table = $('#blogs-list').DataTable({
    columnDefs: [
        ...
        render: function(data, type, row, meta){
            BlogContent = row.Blog_Content + " " +row.Paragraph_2 +" "+ row.Paragraph_3
            content = BlogContent.length
            if(content > 40){
                return ('<span>' + BlogContent.substr( 0, 25 ) + '... </span>' +
...
$(document.body).on("click", ".edit-blog", function(){
    ...
    $("textarea[name='para_2']").val(blog.Paragraph_2);
    $("textarea[name='para_3']").val(blog.Paragraph_3);
    $("textarea[name='quote']").val(blog.Special_Quote);
