...
class Blogs{
    ...
    public function getBlogs(){
        $result = $this->con->query("SELECT `Blog_ID`, blog_cats.Blog_Cat_ID as Blog_Cat_ID, `Blog_Title`, `Blog_Cat_Name` as Category, `Blog_Content`, `Blog_Image_S`, `Blog_Image_B`, `Date`, `Time`, `Author` FROM blogs, blog_cats
    ...
    //Add Blog DB Query
    public function addBlog($title, $date, $time, $author, $category_id, $content, $file1, $file2){
                    ...
                    if(($file2['size']) <= (2097152)){
                        ...
                        if (move_uploaded_file($file1['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/ncmea/admin/assets/img/blogs/".$uniqueImageName1)) {
                            if (move_uploaded_file($file2['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/ncmea/admin/assets/img/blogs/".$uniqueImageName2)) {
                                $q = $this->con->query("INSERT INTO `blogs`(`Blog_Title`, `Date`, `Time`, `Author`, `Blog_Cat_ID`, `Blog_Content`, `Blog_Image_B`, `Blog_Image_S`) VALUES ('$title', '$date', '$datetime', '$author', '$category_id', '$content', '$uniqueImageName1', '$uniqueImageName2')");
                                if ($q) {
    ...
    public function editBlogWithoutImage($id, $title, $date, $time, $author, $category_id, $content){
        if($id != null){
            $datetime = $date ." ". $time;
    ...
    public function editBlogWithSmallImage($id, $title, $date, $time, $author, $category_id, $content, $file){
        $fileName = $file['name'];
    ...
    public function editBlogWithBigImage($id, $title, $date, $time, $author, $category_id, $content, $file){
        $fileName = $file['name'];
		$fileNameAr= explode(".", $fileName);
		$extension = end($fileNameAr);
		$ext = strtolower($extension);
    ...
    public function editBlogWithBigAndSmallImage($id, $title, $date, $time, $author, $category_id, $content, $file1, $file2){
        $fileName1 = $file1['name'];
		$fileNameAr1= explode(".", $fileName1);
		$extension1 = end($fileNameAr1);
		$ext1 = strtolower($extension1);
...
if (isset($_POST['add_blog'])){
    ...
    if(empty($content)){
        ...
        exit();  
    }else if (empty($_FILES['blog_s_image']['name'])){
        echo json_encode(['status'=> 303, 'message'=> 'Add Blog Preview Image']);
        exit();
    }
    else if (empty($_FILES['blog_b_image']['name'])){
        echo json_encode(['status'=> 303, 'message'=> 'Add Blog Content Image']);
        exit();
    }else{
        $result = $b->addBlog($title, $date, $time, $author, $category_id, $content, $_FILES['blog_b_image'], $_FILES['blog_s_image']);
...
if (isset($_POST['edit_blog'])){
    ...
    if(empty($content)){
        ...
        if(isset($_FILES['blog_s_image']['name']) && !empty($_FILES['blog_s_image']['name'])){
            $result = $b->editBlogWithSmallImage($id, $title, $date, $time, $author, $category_id, $content, $_FILES['blog_s_image']);
        }else if(isset($_FILES['blog_b_image']['name']) && !empty($_FILES['blog_b_image']['name'])){
            $result = $b->editBlogWithBigImage($id, $title, $date, $time, $author, $category_id, $content, $_FILES['blog_b_image']);
            ...
            $result = $b->editBlogWithBigAndSmallImage($id, $title, $date, $time, $author, $category_id, $content, $_FILES['blog_b_image'], $_FILES['blog_s_image']);
        }else{
            $result = $b->editBlogWithoutImage($id, $title, $date, $time, $author, $category_id, $content);
        }
        echo json_encode($result);
        exit();
    }
